...
class SendToCivis(MetalNode):
    ...
    def setup(self):
        ...
        if self.via_staging_table:
            ...
            fut = civis.io.query_civis(sql_query, database=self.database, client=self.api_client)
            _ = fut.result()
            sql_query = (
...
class FindValueInRedshiftColumn(MetalNode):
    ...
    def generator(self):
        ...
        fut = civis.io.query_civis(create_statement, database=self.database, client=self.api_client)
        result = fut.result()
        value = (
