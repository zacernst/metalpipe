...
from metalpipe.modelling.treehorn_tokenizer import tokens
...
class PythonFunction(UserDefinedFunction):
    ...
    def __call__(self, *myargs):
        print("in __call__")
        myargs = tuple(
            i if not isinstance(i, (treehorn.TracedPrimitive,)) else i.thing
            for i in myargs
        )
...
def load_query_text_to_logic(query_text):
    """
    Takes the text of a .mtl query, parses it, and loads the facts into the Datalog engine.

    Args:
        query_text (str): String of .mtl query
    Returns:
        dict, dict: Dictionary mapping functions names to functions, and unique property names to properties.
    """
...
# Next: starting with rows as dictionaries mapping column names to values,
#       parse simple SELECT AS queries into logic engine.
